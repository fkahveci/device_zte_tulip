...
struct preview_fixup_entry {
    const char* orig_val;
    const char* repl_val;
};
static preview_fixup_entry preview_fixup_table[] = {
    { "800x480", "1280x960" },
    { "864x480", "1280x960" },
};
        ...
        const char *parameters)
{
    ...
    if (preview_size) {
        size_t idx;
        for (idx = 0; idx < ARRAY_SIZE(preview_fixup_table); ++idx) {
            const preview_fixup_entry* entry = &preview_fixup_table[idx];
            if (!strcmp(preview_size, entry->orig_val)) {
                ALOGI("%s: preview-size %s -> %s", __func__,
                      entry->orig_val, entry->repl_val);
                cp.set("preview-size", entry->repl_val);
